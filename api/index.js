...
let stories = deepClone(defaultStories);
...
function checkBadges() {
  ...
  badges.forEach(b => {
    if (!b.unlocked) {
      ...
      if (conditions[k] >= parseInt(v)) b.unlocked = true;
    }
  });
  return badges;
}
...
function mockAIResponse(userMsg, roleKey) {
  ...
  if (msg.includes('阅读') || msg.includes('reading') || msg.includes('技巧')) {
...
export default async function handler(request) {
  ...
  try {
    ...
    if (pathname === '/api/reset' && request.method === 'POST') {
      ...
      stories = deepClone(defaultStories);
